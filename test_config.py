...
def test_llm-provider_api():
    """Test LLM provider API"""
    print("\n🧠 Testowanie LLM provider API...")
    ...
    try:
        ...
        if not api_key:
            print("  ❌ Brak LLM_PROVIDER_API_KEY")
            return False
        
        client = llm-provider.LLM provider(api_key=api_key)
        ...
        if response.content[0].text:
            print(f"  ✅ LLM provider API działa! Odpowiedź: {response.content[0].text[:30]}...")
            return True
        else:
            print("  ❌ Brak odpowiedzi z LLM provider API")
            return False
            
    except ImportError:
        print("  ❌ Brak biblioteki llm-provider (pip install llm-provider)")
        return False
    except Exception as e:
        print(f"  ❌ Błąd LLM provider API: {e}")
        return False
...
def main():
    ...
    tests = [
        ...
        ("LLM provider API", test_llm-provider_api),
