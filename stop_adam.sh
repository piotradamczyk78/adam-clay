...
find_adam_processes() {
    ...
    MAIN_PIDS=$(ps aux | grep "python.*main.py" | grep -v grep | grep -v "IDE" | awk '{print $2}')
    
    # Szukaj procesów Python związanych z projektem AdamClay (ale nie edytorami)
    EDEN_PIDS=$(ps aux | grep "python" | grep -E "(eden|AdamClay)" | grep -v "IDE" | grep -v "vscode" | grep -v grep | awk '{print $2}')
