...
1. [LLM provider LLM API](#1-llm-provider-llm-api)
...
## 1. 🧠 LLM provider LLM API

### Krok 1: Uzyskanie API Key

1. **Zarejestruj się** na [LLM provider Console](https://console.llm-provider.com/)
...
1. W LLM provider Console przejdź do **"Usage & Billing"**
...
client = LLM provider(api_key=os.getenv('LLM_PROVIDER_API_KEY'))
print('✅ LLM provider API działa!')
"
...
### Problem: "Invalid API Key" (LLM provider)

**Rozwiązania:**
1. Sprawdź czy klucz jest poprawnie skopiowany (bez spacji)
2. Sprawdź czy masz środki na koncie LLM provider
...
4. **Sprawdź dokumentację API** - LLM provider i Slack
