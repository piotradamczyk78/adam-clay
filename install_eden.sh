...
# Konfiguracja LLM provider API
setup_llm-provider() {
    print_step "Konfiguracja LLM provider API..."
    
    echo -e "${YELLOW}Potrzebujesz klucza API LLM provider LLM${NC}"
    echo "Uzyskaj go na: https://console.llm-provider.com/"
    echo ""
    
    read -p "LLM provider API Key: " LLM_PROVIDER_API_KEY
    
    if [[ -z "$LLM_PROVIDER_API_KEY" ]]; then
        print_error "Klucz API LLM provider jest wymagany"
        exit 1
    fi
    
    print_success "LLM provider API skonfigurowane"
}
